...
class Mutfak extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(title: Text("Mutfak Eşyaları")),
        body: Center(
  ...
  List<Kelime> veriKaynaginiHazirla() {
    List<Kelime> gecici = [];
    for (int i = 0; i < 21; i++) {
      var kelimeAdi = Mutfaks.KELIME_ADLARI[i];
      var kelimeResmi = Mutfaks.KELIME_RESIMLERI[i];
      var kelimeAnlami = Mutfaks.KELIME_ANLAMLARI[i];
      var kelimeSesi = Mutfaks.KELIME_SESLERI[i];
